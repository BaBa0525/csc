...
void hmac_sha1(uint8_t const* k, size_t klen, uint8_t const* data, size_t dlen,
               uint8_t* digest, size_t* digelen) {
    SHA_CTX ictx, octx;
...
ssize_t hmac_sha1_96(uint8_t const* k, size_t klen, uint8_t const* data,
                     size_t dlen, uint8_t* digest) {
    size_t digelen = HMAC96AUTHLEN;
    hmac_sha1(k, klen, data, dlen, digest, &digelen);

    if (digelen != HMAC96AUTHLEN) return -1;
