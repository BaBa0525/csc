...
void ipsec_hijack(char* INTERFACE) {
    ...
    *state = WAIT_PKT;
     ...
     * the header
     * state WAIT_SECRET: after sending message to server, you will start to
     * wait for secret sent by server
     * state SEND_ACK: after successfully get the secret, you shoud send ACK
     * back to server
     */
    ...
    *test_for_dissect = true;
    ...
    while (1) {
        ...
        if (*state == SEND_ACK) {
             ...
             * been received successfully. So we also have to reply a ACK to
